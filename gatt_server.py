...
class Application(dbus.service.Object):
    ...
    def __init__(self, bus):
        ...
        self.add_service(GlucoseService(bus, 1))
...
class GlucoseService(Service):
    ...
    def __init__(self, bus, index):
        Service.__init__(self, bus, index, self.Glucose_UUID, True)
        self.add_characteristic(GlucoseLevelCharacteristic(bus, 0, self))


class GlucoseLevelCharacteristic(Characteristic):
   
    Glucose_LVL_UUID = '2A18'

    def __init__(self, bus, index, service):
        Characteristic.__init__(
                self, bus, index,
                self.Glucose_LVL_UUID,
                ['read', 'notify'],
                service)
        ...
        self.highf=151
        self.highs=255
        GObject.timeout_add(2000, self.glucose_levelhigh)
        GObject.timeout_add(5000, self.glucose_levellow)
        GObject.timeout_add(3000, self.glucose_levelmid)
    ...
    def notify_battery_level(self):
        ...
        self.PropertiesChanged(
                GATT_CHRC_IFACE,
                {'Value': [dbus.Int32(self.level)] }, [])
        
        

    def glucose_levellow(self):

        for i in range(50) :
            self.level= random.randint(self.lowf,self.lows)
        print('Glucose level: ' + repr(self.level))
        self.notify_battery_level()
        #self.mydb.writeToDB(self.level)
        return True

    def glucose_levelmid(self):

        for i in range(40):
            self.level= random.randint(self.midf,self.mids)
        print('Glucose level: ' + repr(self.level))
        self.notify_battery_level()
        #self.mydb.writeToDB(self.level)
        return True    
    ...
    def glucose_levelhigh(self):

        for i in range(50):
            self.level= random.randint(self.highf,self.highs)
        print('Glucose level: ' + repr(self.level))
        self.notify_battery_level()
        #self.mydb.writeToDB(self.level)
        return True
    ...
    def ReadValue(self, options):
        print('Battery level read: ' + repr(self.level))
        return [dbus.Int32(self.level)]
